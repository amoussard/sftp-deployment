...
DeploymentManager.prototype.uploadCurrentFile = function() {
    ...
    configFactory.loadConfig(configPath)
        ...
        .then(function(config, connection) {
            console.log(atom.workspace.activePaneItem.buffer.file.path);
        });
};

/**
 * Upload the open tab file on save
 */
DeploymentManager.prototype.uploadCurrentFileOnSave = function() {
  var self = this;
  console.log('uploadCurrentFileOnSave');
  ...
  configurationFileExist(function () {
    ...
    configFactory.on("configuration_ready", function(config) {
      ...
      if (self.config.getUploadSave() === true) {
