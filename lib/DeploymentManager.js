...
var bindEvents = function (connection) {
  ...
  // New events
  connection.on('file_upload_begin', function (file) {
    notifyObservers("file_upload_begin", file);
  });
  connection.on('file_download_begin', function (file) {
    notifyObservers("file_download_begin", file);
  });

  connection.on('create_remote_directory', function (file) {
    notifyObservers("create_remote_directory", file);
  });

  connection.on('create_remote_directory_error', function (file) {
    notifyObservers("create_remote_directory_error", file);
  });

  connection.on('file_upload_in_progress', function (file) {
    notifyObservers("file_upload_in_progress", file);
  });
  connection.on('file_download_in_progress', function (file) {
    notifyObservers("file_download_in_progress", file);
  });

  connection.on('file_upload_error', function (file) {
    notifyObservers("file_upload_error", file);
  });
  connection.on('file_download_error', function (file) {
    notifyObservers("file_download_error", file);
  });

  connection.on('file_upload_success', function (file) {
    notifyObservers("file_upload_success", file);
  });
  connection.on('file_download_success', function (file) {
    notifyObservers("file_download_success", file);
  });
