module.exports = {
    ...
    connection.sftp(function(err, sftp) {
      ...
      aFiles.forEach(function(file) {
        ...
        connection.exec('mkdir -p ' + path.dirname(file.dest), function (err) {
          if (err) {
            SFTP.createMessage('Upload of : ' + file.src, file.src, 'error');
            throw err;
          }
          ...
          sftp.fastPut(file.src, file.dest, function(err) {
            ...
            SFTP.createMessage('Upload of : ' + file.src, file.src, 'success');
            sftp.end();
          });
        });
      });
    ...
    connection.sftp(function(err, sftp) {
      ...
      aFiles.forEach(function(file) {
        ...
        sftp.fastGet(file.src, file.dest, function(err) {
          if (err) {
            SFTP.createMessage('Download of : ' + file.src, file.src, 'error');
            throw err;
          }
          SFTP.createMessage('Download of : ' + file.src, file.src, 'success');
          sftp.end();
        });
      });
    });
  ...
  createMessage: function(message, file, classes) {
    ...
    var sftpMessages = workspace.find(".sftp-messages");
    if (sftpMessages.length === 0) {
      $('.workspace').append('<div class="sftp-messages"></div>');
      sftpMessages = workspace.find(".sftp-messages");
    }

    sftpMessages.append('<span id="'+md5(file)+'" class="message '+classes+'">'+message+'</span><br/>');

    setTimeout(function() {
      var message = $("#"+md5(file));
      var messages = $(".sftp-messages").children('.message');
      message.remove();
